...
//
// APK:OS carries the Sampler's engines out of this tree — `dist/fx.js`, below
// — because a copy is a fork. It could not carry the panels the same way:
// they are .jsx, and APK:OS has no compiler. So it drew its own, and the desk's
// reverb became four unlabelled sliders over a VARC 444 that has a real
// faceplate sitting right here.
//
...
// The third bundle: the ENGINES alone — the audio layer of libControl/Effects
// and the three SoundSynth files they size themselves against, and nothing
// with a face. PLAN-3315.01.
//
// APK:OS used to fetch these twenty-two files one by one and run each through
// `new Function(source)()` in the desktop's own global scope, which was this
// build's per-file scope wrapper applied at load time, in the one window a
// stuck DSP loop must never share. Its sound pages load this bundle on an
// ordinary script tag instead, on a site of their own.
//
// sources.fx.json must stay a SUBSEQUENCE of sources.json, like the GUI list.
const FX_SOURCES = JSON.parse(readFileSync('sources.fx.json', 'utf8'));
const FX_OUT = 'dist/fx.js';
...
if (check && checkOnly !== undefined && !['app', 'gui', 'fx'].includes(checkOnly)) {
  console.error(`✗ --check ${checkOnly} — the bundles are 'app', 'gui' and 'fx'.`);
  process.exit(2);
}
...
const fxStamp = fingerprintOf(FX_SOURCES);
...
if (check) {
  ...
  const wanted = [[OUT, stamp, 'OA_BUILD_STAMP', 'app', SOURCES],
                  ...
                  [FX_OUT, fxStamp, 'OA_FX_BUILD_STAMP', 'fx', FX_SOURCES]]
...
const fxVersion = versionFor([...FX_SOURCES, 'build.mjs', 'sources.fx.json']);

// Each bundle's stamp and version, and the global its version goes out under.
const BUNDLE_MARKS = {
  OA_BUILD_STAMP: { stamp, version, versionGlobal: 'OA_BUILD_VERSION' },
  OA_GUI_BUILD_STAMP: { stamp: guiStamp, version: guiVersion, versionGlobal: 'OA_GUI_BUILD_VERSION' },
  OA_FX_BUILD_STAMP: { stamp: fxStamp, version: fxVersion, versionGlobal: 'OA_FX_BUILD_VERSION' },
};
...
const compile = (files, out, mark) => {
  const parts = [
    ...
    `window.${mark}="${BUNDLE_MARKS[mark].stamp}";`,
    ...
    `window.${BUNDLE_MARKS[mark].versionGlobal}="${BUNDLE_MARKS[mark].version}";`,
    '(function(){"use strict";',
  ];
  ...
  for (const file of files) {
    ...
    // it throws "already been declared" and that file never runs. APK:OS loaded
    // these sources directly and hit exactly that, and applied this wrapper at
    // load time through `new Function` until it took `dist/fx.js` instead.
    parts.push(`\n/* ---- ${file} ---- */`);
    parts.push('(function(){');
    parts.push(out2);
    parts.push('})();');
  ...
  console.log(`✓ ${out}  ${jsx} jsx + ${plain} js  ${kb(bytesIn)} in -> ${kb(bundle.length)} out  [${BUNDLE_MARKS[mark].stamp}] ${BUNDLE_MARKS[mark].version}`);
};
...
// The engines alone, for APK:OS's sound pages. Third, for the same reason.
compile(FX_SOURCES, FX_OUT, 'OA_FX_BUILD_STAMP');
